...
public class Ages extends AppCompatActivity {
    ...
    private ArrayList<Question> AgequestionList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            ...
            AgeDbHelper dbHelper = new AgeDbHelper(this);
            AgequestionList = dbHelper.getAllQuestions();
            question_total = AgequestionList.size();
            Collections.shuffle(AgequestionList);
            showNextQuestion();
        }else {
            AgequestionList = savedInstanceState.getParcelableArrayList(KEY_QUESTION_LIST);
            question_total = AgequestionList.size();
            question_counter = savedInstanceState.getInt(KEY_QUESTION_COUNT);
            currentQuestion = AgequestionList.get(question_counter - 1);
            ...
            public void onClick(View v) {
                if (!answered){
                    if (op1.isChecked() || op2.isChecked() || op3.isChecked() || op4.isChecked()){
                        ...
                        Toast.makeText(Ages.this, "Please select an answer", Toast.LENGTH_SHORT).show();
    ...
    private void showNextQuestion(){
        ...
        if (question_counter < question_total){
            currentQuestion = AgequestionList.get(question_counter);
    ...
    protected void onSaveInstanceState(@NonNull Bundle outState) {
        ...
        outState.putParcelableArrayList(KEY_QUESTION_LIST,AgequestionList);
